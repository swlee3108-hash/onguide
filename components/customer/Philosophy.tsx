...
export default function Philosophy({ onStartChat }: { onStartChat: () => void }) {
  const cards = [
    {
  ...
  return (
    <section className="px-10 py-16 max-w-[960px] mx-auto">
      ...
      <div className="grid grid-cols-3 gap-6 mb-10">
        {cards.map((c) => (
          <div key={c.num} className="bg-surface border border-subtle rounded-lg p-8 shadow-sm hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200">
            <p className="text-[11px] font-bold tracking-widest text-a-copper mb-3">{c.num}</p>
            <h2 className="text-[22px] font-bold tracking-tight text-t-primary mb-1">{c.title}</h2>
            <p className="text-sm font-medium text-a-caramel mb-3">{c.accent}</p>
            <p className="text-sm text-t-body leading-relaxed">{c.body}</p>
            <div className="flex gap-2 mt-4 flex-wrap">
              {c.tags.map((t) => (
                <span key={t} className="text-[11px] font-medium text-t-muted bg-muted px-2.5 py-1 rounded-xs">{t}</span>
              ))}
            </div>
          </div>
        ))}
